...
class lte_Editor(tk.Tk):
    
    def __init__(self):
        super().__init__()
        # self.pack() # if using tk.Frame, keep this line !!!
        self.title('Check and Edit Lte-file')
    ...
    def setupUI(self):
        ...
        generate_mtr(parameter[1])
        filename = 'Elegant_Process/lte_file/' + parameter[1][2] + '.lte'
        generate_lte(filename, parameter[1])
        ...
        Btn = Btn_def()
        btn1_name = 'Re-load Lte-file:    ' + parameter[1][2] + '.lte'
        # btn1 = Button(self, text='Re-load ELE-file', bg="orange", command=lambda: Btn.load(filename, contents)).pack(side=LEFT)
