...
!-------------------------------------------------------------------------------------
! Comments: This code has been revised by River and Vasily for CASA Beam-Beam Backage
!-------------------------------------------------------------------------------------
...
1000 format(2(1x,es14.7))
  ...
  subroutine phase_Output(nfile,Pts1,Nptlocal,nsamp)
    ...
    print*, "output phase coordinates"
    !nstride = 1
    nstride = nsamp
...
!             print*, recvbuf(1,j),recvbuf(2,j),recvbuf(3,j),recvbuf(4,j),recvbuf(5,j),recvbuf(6,j)
             print*, "output phase coordinates"
          enddo
       enddo
       close(nfile)
...
!****************  JLab-CASA Phase output ************************************************************
    integer :: CASA_i
    character*60 CASA_Phase_out_name
    
    character*10 turn_number
    character*2 Beam_ID

    write(turn_number, '(i6)') iturn
    write(Beam_ID, '(i2)') myid+1
    
    print*, '  ...............        JLab-CASA BeamID-out = ', myid+1, ',      Turn number = ', iturn
    CASA_Phase_out_name = 'CASA_Phase_out_'//'Beam_'//trim(adjustl(Beam_ID))//'_turn_'//trim(adjustl(turn_number))//'.casa'

    open(unit=16, file = CASA_Phase_out_name)
    do CASA_i = 1, Nplocal
       write(16, *) Bpts(1:6,CASA_i)
    enddo
    close(unit=16)
!******************************************************************************************************
